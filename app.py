...
from tensorflow.keras.preprocimport streamlit as st
import numpy as np
import cv2
import json
import requests
...
if "uploaded_images" not in st.session_state:
    st.session_state.uploaded_images = []  # user uploads
if "admin_uploaded_images" not in st.session_state:
    st.session_state.admin_uploaded_images = []  # admin uploads
...
# User Image Upload
# ==============================
def render_user_image_upload():
    st.subheader("📤 Upload Your Brain Scan Images")
    uploaded_files = st.file_uploader(
        "Upload your scan image(s)", type=["jpg", "jpeg", "png"], accept_multiple_files=True, key="user_upload"
    )
    if uploaded_files:
        for file in uploaded_files:
            file_bytes = np.asarray(bytearray(file.read()), dtype=np.uint8)
            image = cv2.imdecode(file_bytes, 1)
            st.image(image, caption=f"🖼 Uploaded by {st.session_state.username}", use_column_width=True)
...
# Admin Image Upload
# ==============================
def render_admin_image_upload():
    st.subheader("📤 Upload Images as Admin")
    uploaded_files = st.file_uploader(
        "Upload image(s) as admin", type=["jpg", "jpeg", "png"], accept_multiple_files=True, key="admin_upload"
    )
    if uploaded_files:
        for file in uploaded_files:
            file_bytes = np.asarray(bytearray(file.read()), dtype=np.uint8)
            image = cv2.imdecode(file_bytes, 1)
            st.image(image, caption=f"🖼 Uploaded by Admin {st.session_state.username}", use_column_width=True)
            st.session_state.admin_uploaded_images.append({
                "username": st.session_state.username,
                "filename": file.name,
                "image": image
            })
        st.success(f"{len(uploaded_files)} image(s) uploaded successfully as admin!")


# ==============================
# Admin View All Uploaded Images
# ==============================
def render_admin_all_uploaded_images():
    st.subheader("🖼 All Uploaded Images (Users + Admin)")
    combined = st.session_state.uploaded_images + st.session_state.admin_uploaded_images
    if not combined:
    ...
    for idx, entry in enumerate(combined):
        ...
        with container:
            st.write(f"Uploader: {entry['username']} | File: {entry['filename']}")
            st.image(entry["image"], use_column_width=True)
            col1, col2 = st.columns(2)
            with col1:
                if st.button(f"✅ Approve_{idx}", key=f"approve_img_{idx}"):
                    st.success(f"Image '{entry['filename']}' approved.")
            with col2:
                if st.button(f"❌ Reject_{idx}", key=f"reject_img_{idx}"):
                    if entry in st.session_state.uploaded_images:
                        st.session_state.uploaded_images.remove(entry)
                    if entry in st.session_state.admin_uploaded_images:
                        st.session_state.admin_uploaded_images.remove(entry)
                    st.error(f"Image '{entry['filename']}' rejected.")
                    st.rerun()
...
def render_admin_dashboard():
    ...
    tabs = st.tabs(
        ["👤 Create User", "🧑‍🤝‍🧑 Manage Users", "📤 Export/Import", "📨 Telegram Settings", "🖼 Image Management"]
    )
    ...
    with tabs[4]:
        render_admin_image_upload()
        render_admin_all_uploaded_images()
...
# -------------------------
# Remaining code (user app, appointments, post-stroke care)
# Your existing code continues below exactly as you provided
# -------------------------

# (Paste here the rest of your user app code including render_user_app, render_appointment_portal, render_admin_appointments, render_post_stroke_care)
...
if not st.session_state.logged_in:
    ...
    if st.session_state.role == "admin":
        ...
        render_user_app()
        render_user_image_upload()  # <-- new user image upload feature
essing.image import img_to_array
