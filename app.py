...
# -------------------------
# Google Drive link for central users.json
# -------------------------
USERS_DRIVE_FILE_ID = "YOUR_DRIVE_FILE_ID_FOR_USERS_JSON"  # upload users.json to Drive and put the ID here
USERS_DRIVE_URL = f"https://drive.google.com/uc?id={USERS_DRIVE_FILE_ID}"

# Download users.json if not exists locally
if not os.path.exists(USERS_FILE):
    with st.spinner("⬇ Downloading users data for multi-device login..."):
        try:
            gdown.download(USERS_DRIVE_URL, USERS_FILE, quiet=False)
            st.success("✅ Users data synced from Drive.")
        except Exception as e:
            st.error(f"Failed to download users.json: {e}")
...
# -------------------------
# Continue all other functions from your original program (render_user_app, appointments, post-stroke care)
# -------------------------
